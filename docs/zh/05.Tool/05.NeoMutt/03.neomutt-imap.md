...
::: details After adding attachments, how do you remove an attachment added by mistake from the `attach` view?
...
::: details Is it possible to revise an email's headers when you are already in `compose` screen?
...
4. Save and exit the editor to return to the `compose` menu.

:::

::: details The Correct Way to Add a Recipient in NeoMutt

After pressing `gr`, you will group reply the current email and be prompted into the text editor for editing the email message. When you are done editing, press `<leader>qa` to exit out the Neovim and go back to the `compose` screen. Now assume you want to add an additional recipient to the distribution list, press:

- `c` to edit the `Cc` list interactively
- Then type: `Lao Hong Jun <hjlao@twineintl.com>`, as your new added recipient
- NeoMutt will properly quote and format the address
- Then press `e` to edit the email body
...
![neomutt-index-view.avif](../../../public/img/2026/neomutt-index-view.avif)
...
![neomutt-pager-view.avif](../../../public/img/2026/neomutt-pager-view.avif)
...
![neomutt-sidebar-view.avif](../../../public/img/2026/neomutt-sidebar-view.avif)

- `<C-j>` to move the highlight down
- `<C-k>` to move the highlight up
- `<Return>` to open the highlight folder

I don't use the sidebar very often. Instead, I use the defined `y` key to show
incoming mailboxes list. Following is an excerpt from `Help for index`:

```txt
y       show incoming mailboxes list
<change-folder>?
```

Because command `change-folder` is mapped to `c` in `index` view, therefore `y` and `c?`
will actually do the same trick, show all mailboxes.
...
- `?` under `index` view will bring up `Help for index`
- `?` under `pager` view will bring up `Help for pager`
