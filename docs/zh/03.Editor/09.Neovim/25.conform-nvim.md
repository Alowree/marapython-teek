...
I use the `conform.nvim` plugin to configure all formatters required inside Neovim. I don't use Mason.
...
在为 Markdown 文档启用了 Prettier 作格式化之后，Prettier 会作以下同义转换：
...
但这会触发 markdownlint-cli2 的 MD049 错误。所以，我们有办法停止 Prettier 的这种同义转换呢？遗憾的是，Prettier 没有提供任何配置选项来关闭这种星号（`*`）和下划线（`_`）之间的转换。
