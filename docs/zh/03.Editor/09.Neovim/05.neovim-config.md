...
2、`lua/alowree/core/init.lua` 导入了 `keymaps.lua` 和 `options.lua` 两个文件

::: details Is this file (`lua/alowree/core/init.lua`) necessary?
...
::: details Why the Lua files under the `plugins` folder are not required explicitly?
...
::: details As the entire `plugins` folder are loaded inside `lazy.nvim,` can I just do the same with the `core` folder, by changing the current configuration to `{ import = "alowree.core", import = "alowree.plugins }`, and then remove the `/lua/alowree/core/init.lua` file?
