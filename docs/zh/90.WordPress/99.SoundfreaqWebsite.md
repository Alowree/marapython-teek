...
Soundfreaq theme color is lime green #284734
...
- 1204 footer menu `Appearance >> Menus → Edit Menus → Company → Black Friday Sale`
...
- /about-us/ title "About Soundfreaq" top margin cut out, but /press-reviews/ title works fine - how can we possibly modify this?
...
1. When at wide screen `min-width: 992px`, all images are squeezed together.
...
`Appearance → Customize → Additional CSS → Goya CSS`

```css
/* Fix thumbnails image display on product pages, when viewed on wide screen */
/* Do not squeeze together */
@media (min-width: 992px) {
.thumbnails-vertical .has-additional-thumbnails .flex-control-thumbs, .thumbnails-vertical .has-additional-thumbnails .woo-variation-gallery-thumbnail-wrapper, .thumbnails-vertical .has-additional-thumbnails .rtwpvg-thumbnail-wrapper {
    width: 440px;
  }
...
/* Left aligned with the main image */
.rtwpvg-images.rtwpvg-has-product-thumbnail .rtwpvg-grid-layout .rtwpvg-thumbnail-wrapper, .rtwpvg-images.rtwpvg-has-product-thumbnail .rtwpvg-wrapper .rtwpvg-thumbnail-wrapper {
    margin-left: 20px;
}
```

1. It takes way too long (dozens of seconds) to load the 6-8 product images.

To remove the error requests by preventing mixed content

The error message `Mixed Content: The page at 'https://soundfreaq.com/product/sound-rise-ii-black/' was loaded over HTTPS, but requested an insecure font 'http://www.soundfreaq.com/public_html/wp-content/fonts/akzidenzgroteskbe-cn-webfont.woff'` tells us that while your website is correctly loaded using https encryption, a specific font file is being called via an insecure http protocol. This creates a mix of secure and insecure content on the same page, which browsers flag as a security risk.
...
@font-face {
		...
		src: url(http://www.soundfreaq.com/public_html/wp-content/fonts/akzidenzgroteskbe-cn-webfont.woff);
		font-weight: normal;
}
```
...
@font-face {
		font-family: akzidenz;
		src: url(https://www.soundfreaq.com/public_html/wp-content/fonts/akzidenzgroteskbe-cn-webfont.woff);
		font-weight: normal;
}
```

This code block is actually not used across the website, thereofore it is now commented out in the Additional CSS field.

To correct the custom font implementation

a. remove the broken link as reported by error message `GET https://use.typekit.net/Akziden.css?ver=6.8.3 net::ERR_ABORTED 404 (Not Found)` by deleting "Akziden" from field `Project ID`:

`Appearance → Customize → Theme Styles → Typography`

Main Font Source → Adobe Typekit

Project ID: Akziden (delete this and leave it blank)

font-family: akziden

b. Upload Font via the "Use Any Font" plugin

https://soundfreaq.com/wp-content/uploads/useanyfont/4660akziden.woff2

c. Add custom css code to `Appearance → Customize → Additional CSS`

```css
body {
	/* Introduced by the "Use Any Font" plugin */
	font-family: 'akziden' !important;
}
```

redirect visitors from homepage to boxing day specials

To enable the redirection, add the following code snippet:
...
add_action('wp_head', function() {
  ...
  if ( is_front_page() ) {
    ...
    (function() {
      ...
      var hasVisited = document.cookie.indexOf('home_visited=true') > -1;

      if (!hasVisited) {
          // 1. Set the cookie for 7 days (max-age in seconds)
          document.cookie = "home_visited=true; max-age=604800; path=/";

          // 2. Redirect to your promotions page
          window.location.href = "/the-gift-of-great-sound";
      }
    })();
    </script>
    <?php
...
To disable the redirection, simply pause the code snippet.
