...
::: details Does the Sound Rise II support automatic daylight savings time change?

Yes, you are right that the Soundfreaq Sound Rise II supports automatic Daylight Saving Time (DST) adjustment.

There is a slide switch on the back of the alarm clock with three settings:
`A`: Enable automatic DST adjustment
`O`: Disable automatic DST adjustment
`I`: Enable DST manually to add one hour to the time

If you are using the Sound Rise II in the US and want automatic DST, simply set the switch to position `A`.
