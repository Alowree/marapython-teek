...
Vim Challenge: To edit line 2-5 into the same format as that of line 1
...
I saw Sylvan Franklin achieved the same result on his Linux by running command `:'<'>norm _i{src=^[f,i}` after the visual selection, but the same trick did not work on my Windows machine. Any ideas why the difference?
