...
Vim Challenge: To edit the content of line 2-5 into the same format as that of line 1 in the configuration file below
...
Solution: The substitute command

`:s/\(.*\),/{src = \1},/`

I saw Sylvan Franklin achieved the same result on his Linux by running command `:'<'>norm _i{src=^[f,i}` after the visual selection of line 2-5, but the same trick did not work on my Windows machine. Any ideas why the difference?
