...
With my current configurations, pressing `<C-o>zz` in Insert mode does not redraw the screen as expected, but goes back to Insert mode and append "zz" literally.

The root cause is that `which-key` is configured to activate whenever you press a key that is a prefix for another keymap (in this case, `z` is a prefix for your `za` fold toggle). This activation introduces a tiny delay to see if you'll complete the sequence. This delay is what causes the temporary Normal mode from `<C-o>` to expire, leading to `zz` being inserted as text.
