...
Currently I have a couple of problems with the spell check inside Neovim: a, pressing `[s` or `]s` appears to skip apparent spell errors inside a list, ordered or unordered; and b, the style highlight of inline code appears to overwrite that of the spell error, therefore you don't see that red wavy line beneath the miss-spelled words, when they are inside the inline code.
