...
我现在使用 macOS，所以也在 `~/.zshrc` 添加类似的函数：

```zsh
# I keep several Neovim configurations under $HOME directory for learning purposes
# Function to select different Neovim configurations interactively
function nvims() {
    local -a items=("default" "nvim-josean" "nvim-from-scratch")
