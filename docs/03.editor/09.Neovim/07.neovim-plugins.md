...
- 左边使用 nvim-tree 生成的侧边栏，以树形目录进行导航
- 顶部使用一个 bufferline.nvim 生成的标签页面，进行 tab 窗口管理

Vim/Neovim 中的 tab 标签页面，跟 VS Code 中的 tab 标签页面，外在表现形式有一点类似，但本质上却又不完全相同。

- VS Code 中的标签页面就是 **一个** 打开的文件
- Vim/Neovim 中的标签页面是 **多个** buffer 的容器，表面露出一个，后面可以隐藏很多

后来发现，虽然安装了 bufferline.nvim 插件，但是从来没有使用过 tab 相关功能，停用之后也没有任何不适。
...
这一系列关于 LSP 的插件是根据 Mr Jakob 的 nvim-from-scratch 系列视频的第 4 集，做了重新的配置。

在 Lua 配置文件内，会生成 "Undefined global `vim`" 警告信息。
...
- This uses `gcc` in the background, which is handled by the
  [nvim-mini/mini.comment](https://github.com/nvim-mini/mini.comment){:target="\_blank"}
  - So make sure that plugin is installed, comes with lazyvim.org distro by default
- Insert filename with path by `<C-z>`
