...
- **Variable Expansion**:
  `l~s "Invoice from $USER"`, The `$USER` variable is substituted with the current user's name before searching.
- **Command Substitution**:
  <code>l~d "<\`date -d '1 month ago' +%Y%m%d\`"</code>, The date command inside the backticks (`date -d '1 month ago' +%Y%m%d`) is executed, and its output (e.g., `20250914`) is inserted into the search string. The resulting search is `l~d "<20250914"`.
