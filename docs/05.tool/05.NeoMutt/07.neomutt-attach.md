...
permalink: /pages/neomutt-attach
categories:
  - Tool
  - CLI
---
...
在 NeoMutt 内，按下 `m` 即可创新一封新的邮件。在填写完邮件的头部信息之后，比如收件人、抄送人、主题等信息，NeoMutt 会打开设定的文本编辑器，比如我设定的是 Neovim，在既定的邮件模板上进行邮件主体的编辑。当邮件主体内容编辑完成之后，我们按 `:w` 保存、按 `:q` 退出 Neovim 编辑器。这时 NeoNutt 进入 `compose` 状态，类似位于一个待发送状态，按下 `y` 即可发送邮件。
...
当 NeoNutt 位于 `compose` 窗口时，按下 `C-a` 即可打开文件选择窗口，按 `Tab` 可选择多个文件，按回车确认。
